...
def select_topic_by_env():
    if ENV_MODE == 'dev':
        return parser.get('kafka_topics','book_purchasing_kafka_topic_dev')
    else:
        return parser.get('kafka_topics','book_purchasing_kafka_topic_prod')
...
# KAFKA TOPICS
BOOK_PURCHASING_KAFKA_TOPIC = select_topic_by_env()

# KAFKA BROKERS 
BROKER_SERVER = select_broker_server_by_env()